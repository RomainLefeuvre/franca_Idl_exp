...
class Gen_DTO {
	...
	def private generateData (List<FTypeCollection> typeCollections,String basePackageName)'''
	 «generateHeader(basePackageName)»
	 	«FOR typeCollection : typeCollections»
	 		«typeCollection.generateTypeCollection»		 
	 	«ENDFOR»
	 	
	 }
	'''
	
	def private generateTypeCollection(FTypeCollection collection)'''
	...
	def dispatch generateType(FType type){
		throw new Exception("You should not pass there ! You need to handle the missing concrete type")
	}
	
	def dispatch generateType(FStructType type)'''
		class «type.name» {
			«FOR element : type.elements»
			«element.type.generateTypeRef()» «element.name»
			«ENDFOR»
		}
	'''
	
	def private generateTypeRef (FTypeRef it) {
		if(derived!=null){
			derived.name
		}else{
			switch(predefined){
